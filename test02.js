...
//
// This application retreives information about a URL
  ...
  response.on('end', function () {
    ...
    if (response.statusCode > 300 && response.statusCode < 400 && response.headers.location) {
      ...
      if ( target.substring(0,1) == '/'){
        target=options.protocol+'://'+options.hostname+response.headers.location;
      }
      console.log('\n redirected to: ' + target);
      if (target != latestLocation && redirectDepth < 5){
        serverInfo(target);
      }
    }
  });
